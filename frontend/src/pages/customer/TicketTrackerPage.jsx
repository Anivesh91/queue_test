...
export const TicketTrackerPage = () => {
  ...
  useEffect(() => {
    ...
    // Reconnect handler: Page 19 "socket reconnect -> GET ticket current state -> replace client state"
  ...
  if (error || !ticketData) {
    return (
      <div className="max-w-md mx-auto px-4 py-16 text-center">
        ...
        <p className="text-xs text-slate-500 mt-1">{error || 'This ticket token is invalid or expired.'}</p>
        <Link
          to="/customer"
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8 sm:py-12">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden relative">
        {/* Top Header */}
        <div className="px-6 pt-6 pb-4 bg-slate-50/70 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white">
              <TicketIcon className="w-4 h-4" />
            </div>
            <div>
              <div className="text-xs font-bold text-slate-800">{organization?.name}</div>
              <div className="text-[11px] text-slate-500">{service?.name}</div>
            </div>
          </div>
        ...
        <div className="p-8 text-center bg-gradient-to-b from-white to-slate-50/50">
          ...
          <div className="text-5xl sm:text-6xl font-black font-mono text-slate-900 tracking-tight mt-2 mb-3">
            {ticket.ticketNumber}
          </div>
        ...
        <div className="relative flex items-center justify-between px-2">
          <div className="w-5 h-5 rounded-full bg-slate-50 -ml-4 border-r border-slate-200" />
          <div className="flex-1 border-t-2 border-dashed border-slate-200 mx-2" />
          <div className="w-5 h-5 rounded-full bg-slate-50 -mr-4 border-l border-slate-200" />
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          {status === 'CALLED' && (
            <div className="text-center py-4 space-y-3">
              <div className="w-16 h-16 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center mx-auto animate-live-glow">
                <BellRing className="w-8 h-8" />
              </div>
              <h3 className="text-xl font-bold text-slate-900">Your Number is Called!</h3>
          ...
          {status === 'SERVING' && (
            <div className="text-center py-4 space-y-3">
              ...
              <h3 className="text-xl font-bold text-slate-900">Service in Progress</h3>
              <p className="text-xs text-slate-600 max-w-xs mx-auto">
          ...
          {status === 'COMPLETED' && (
            <div className="text-center py-4 space-y-3">
              ...
              <h3 className="text-xl font-bold text-slate-900">Service Completed</h3>
              <p className="text-xs text-slate-600 max-w-xs mx-auto">
                Thank you for using QueueLess! Your turn has concluded.
              </p>
              <Link
                to="/customer"
                className="inline-block mt-3 px-4 py-2 text-xs font-semibold text-blue-600 bg-blue-50 rounded-xl hover:bg-blue-100"
              >
                Join Another Queue
              </Link>
            </div>
          ...
          {status === 'CANCELLED' && (
            <div className="text-center py-4 space-y-3">
              ...
              <h3 className="text-lg font-bold text-slate-900">Ticket Cancelled</h3>
          ...
          {status === 'NO_SHOW' && (
            <div className="text-center py-4 space-y-3">
              ...
              <h3 className="text-lg font-bold text-slate-900">Marked as No-Show</h3>
              <p className="text-xs text-slate-500 max-w-xs mx-auto">
